...
- CSS Grid layout: fixed 220px project-name column, 80px Total WIP column, one `1fr` column per age bin, then two `1fr` columns for orphan counts
...
- Orphan cells show a count linking to Jira search, or `—` if zero
